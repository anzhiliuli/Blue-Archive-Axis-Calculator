...
- 持续回费功能：设置持续回费效果，影响学生的回费速度，以卡片形式展示在关联设置列表中
- 数据筛选：支持按条件筛选数据项
- 分页功能：支持分页显示大量数据项
...
- 技能按时间排序：新增或修改数据行后自动插入到对应时间位置，费用、时间间隔、剩余费用按正确顺序重新计算
- 持续回费卡片化：持续回费设置以紫色卡片显示在关联设置列表中，与关联规则明确区分，支持单独删除（可撤销）和绑定行失效提醒
- 删除规则即时生效：删除关联规则后，数据表费用数据立即恢复，时间轴和费用曲线同步刷新
- 撤销/重做可靠性修复：初始化等复杂操作支持一步撤销，不再出现撤不干净或数据错乱
- 多项体验修复：时间筛选报错、编辑弹窗未确认数值写入数据、导入数据校验、编号冲突等问题修复
- 界面优化：时间轴卡片样式补全、分页按钮禁用态、选中行状态保持、游戏模式按钮状态同步
- 稳定性与性能：模拟计算防卡死保护，长轴多规则场景计算提速
- 安全加固：输入内容转义处理，外部链接安全校验
- 修复依赖安装失败问题（依赖清单中存在不存在的库版本声明）
